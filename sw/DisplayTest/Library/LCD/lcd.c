...
//    config.spi_sz=size;
//    if(size==mode_14bit){
//      LCD_HANDLE.Init.DataSize = SPI_DATASIZE_14BIT;
...
static void LCD_WriteCommand(uint16_t *cmd, uint8_t argc)
{
  ...
  if(argc){
  LCD_PIN(LCD_DC,SET);
    HAL_SPI_Transmit(&LCD_HANDLE, (cmd+1), argc-1, HAL_MAX_DELAY);
  }
#ifdef LCD_CS
  LCD_PIN(LCD_CS,SET);
#endif
...
static void LCD_WriteData(uint8_t *buff, size_t buff_size) {
  ...
  LCD_PIN(LCD_DC,SET);
	...
	while (buff_size > 0) {
		uint16_t chunk_size = buff_size > 65535 / 2 ? 65535 / 2 : buff_size;
#ifdef USE_DMA
...
#ifdef LCD_DC
  uint8_t data[2] = {DC_BIT_DATA|( color >> 8), DC_BIT_DATA|(color & 0xFF)};
  LCD_PIN(LCD_DC,SET);
#else
...
#else
    //setSPI_Size(mode_9bit);                                                          // Set SPI to 8 bit
#endif
    return NULL;
  }
...
#else
    //setSPI_Size(mode_14bit);                                                          // Set SPI to 16 bit
#endif
#ifdef LCD_DC
  LCD_PIN(LCD_DC,SET);
#endif
...
void LCD_DrawImage(uint16_t x, uint16_t y, UG_BMP* bmp)
{
  ...
  setDMAMemMode(mem_increase, mode_14bit);                                                            // Set SPI and DMA to 16 bit, enable memory increase
#else
	//setSPI_Size(mode_9bit);                                                                            // Set SPI to 16 bit
#endif
  }
